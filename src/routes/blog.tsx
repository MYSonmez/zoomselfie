...
export default function Blog() {
  ...
  return (
    <div data-no-translate className="overflow-hidden bg-[#f4f0e8] text-zinc-950">
      <section className="relative min-h-[100svh] overflow-hidden bg-[#f4f0e8] pb-20 pt-28 sm:pt-32 lg:pt-36">
        ...
        <div className="container-page relative">
          <header className="grid gap-5 border-b border-black/15 pb-7 lg:grid-cols-[1fr_.62fr] lg:items-end">
            <div>
              <p className="text-xs font-black uppercase tracking-[.24em] text-amber-600">{copy.eyebrow}</p>
              <h1 className="mt-4 max-w-4xl text-[clamp(2.85rem,4.4vw,4.6rem)] font-extrabold leading-[.94] tracking-[-.06em]">{copy.title}</h1>
            </div>
            <p className="max-w-xl text-sm leading-6 text-zinc-600 lg:justify-self-end lg:text-base lg:leading-7">{copy.intro}</p>
          </header>

          <div className="mt-6 flex flex-col gap-5 sm:flex-row sm:items-center sm:justify-between">
            <p className="text-[10px] font-black uppercase tracking-[.22em] text-zinc-400">{copy.indexLabel} · {copy.latest}</p>
            <div className="flex max-w-full gap-2 overflow-x-auto pb-1 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden" aria-label={copy.categoriesLabel}>
              {blogCategoryKeys.map((item) => (
                <button key={item} type="button" onClick={() => setCategory(item)} className={cn("whitespace-nowrap rounded-full border px-4 py-2 text-[11px] font-bold transition", category === item ? "border-zinc-950 bg-zinc-950 text-white" : "border-black/12 bg-white/65 text-zinc-600 hover:border-black/35 hover:text-zinc-950")}>
          ...
          <div className="mt-7 grid gap-x-6 gap-y-12 md:grid-cols-2 lg:grid-cols-3">
            {visiblePosts.map((post) => <ArticleCard key={post.id} post={post} locale={selectedLocale} readLabel={copy.read} minutesLabel={copy.minutes} />)}
          </div>
...
function ArticleCard({ post, locale, readLabel, minutesLabel }: { post: LocalizedBlogPost; locale: BlogLocale; readLabel: string; minutesLabel: string }) {
  return (
    <article className="group">
      <Link to={`/blog/${locale}/${post.slug}`} className="block">
        <div className="relative aspect-[16/10] overflow-hidden rounded-[1.5rem] bg-zinc-100">
          <Image src={post.image} alt={post.imageAlt} loading="lazy" sizes="(max-width: 768px) 100vw, (max-width: 1024px) 50vw, 33vw" className="absolute inset-0 h-full w-full object-cover transition-transform duration-700 ease-out group-hover:scale-[1.025]" />
        ...
        <div className="mt-5">
          ...
          <h3 className="mt-3 text-xl font-extrabold leading-[1.15] tracking-[-.03em] transition-colors group-hover:text-amber-600 sm:text-2xl">{post.title}</h3>
          <p className="mt-2.5 max-w-2xl text-sm leading-6 text-zinc-600">{post.excerpt}</p>
